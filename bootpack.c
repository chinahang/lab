...
void HariMain(void)
{
	...
	memtotal = memtest(0x00400000, 0xfffffff);
	memman_init(memman);
	...
	tss_b.fs = 1 * 8;
	tss_b.gs = 1 * 8;
	*((int*)0x0fec) = (int)sht_back; /*随便找的内存地址*/
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (i == 2) {
				...
				boxfill8(sht_win->buf, sht_win->bxsize, cursor_c, cursor_x, 29, cursor_x + 7, 43);
