...
struct SHTCTL *shtctl_init(struct MEMMAN *memman, unsigned char *vram, int xsize, int ysize)
{
	...
	for (i = 0; i < MAX_SHEETS; i++) {
		...
		ctl->sheets0[i].ctl = ctl; /*structor inside structor*/
	}
err:
	return ctl;
}
...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	int h, old = sht->height; /*record the height before set*/
...
{
	if (sht->height >= 0) { /* if showing,then update in new sheet */
